import HeaderBramptonMap from "./components/HeaderBramptonMap.js";
import './style/brampton_map.css';
import { useEffect } from "react";

function BramptonMap() {
  useEffect(() => {
    async function fetchDocuments() {
      ...
      let num = 0;
      for (const file of data[5]) {
        const response_fetch = await fetch('http://localhost:3001/api/contents/brampton_map', {
          method: 'POST',
          headers: {
  ...
  return (
    <>
      ...
      <div style={{ display: 'flex' }}>
        ...
        >
          <a style={{ color: 'black', fontSize: '60px', backgroundColor: '#fff', display: 'flex', alignItems: 'center', height: '200px', border: '4px solid black', cursor: 'pointer', justifyContent: 'center', textDecoration: 'none' }} href="/brampton">BACK</a>
        </div>
      </div>
    </>
  );
