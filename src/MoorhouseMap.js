import HeaderMoorhouseMap from "./components/HeaderMoorhouseMap";
import './style/moorhouse_map.css';
import { useEffect } from "react";

function MoorhouseMap() {
  useEffect(() => {
    async function fetchDocuments() {
      ...
      let num = 0;
      for (const file of data[6]) {
        const response_fetch = await fetch('http://localhost:3001/api/contents/moorhouse_map', {
          method: 'POST',
          headers: {
  ...
  return (
    <>
      ...
      <div style={{ display: 'flex' }}>
        ...
        >
          <a style={{ color: 'black', fontSize: '60px', backgroundColor: '#fff', display: 'flex', alignItems: 'center', height: '200px', border: '4px solid black', cursor: 'pointer', justifyContent: 'center', textDecoration: 'none' }} href="/moorhouse">BACK</a>
        </div>
      </div>
    </>
  );
